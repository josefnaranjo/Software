from dictionarySortedList import *
from abstract import *
from game import Game
import csv

class GameInventorySort(GameInvAbstract):
  def __init__(self):
    self.s = SortedListDict()
    
  # Search for a title
  def search_title(self, title):
    
    return self.__game_sort.getitem(title)
  ...
  def create_inventory(self, videogamelist):
    with open(videogamelist, "r") as csvfile:
      ...
      for rows in csvreader:
        ...
        self.s.additem(game.get_title(), game)
  ...
  def __str__(self):
    ...
    for game in self.__game_sort:
      gameInv_Sort += "#" + str(count) + " -> " + str(game) + "\n"
      count += 1
